 ...
 *
 * 2. The `get_bucket_index` method takes a key as input and calculates a bucket index using the `modulo` operator (`%`) by dividing the key by the `capacity`. This maps the key to a specific bucket within the buckets array.
 *
 * 3. The `add` function is used to adding a new key to the hash table. This function takes a key as input.
 * - 3.1: We call `get_bucket_index` method to determine the bucket index for the key.
 *
 ...
 * - 4.1: We call `get_bucket_index` method to determine the bucket index for the key.
 *
 * - 4.2: We use the retain method on the bucket array to create a new array containing only elements that are not equal to the removed key. This effectively removes the key from the set if it exists. We then update the original bucket array with the filtered array.
 *
 * 5. The contains function is used to check if the key is existed on the hash set. This function takes a key as input.
 * - 5.1: We call `get_bucket_index` method to determine the bucket index for the key.
 *
 * - 5.2: We use the contains method on the bucket array to check if the key exists within the bucket. We return `true` if the key is found in the bucket, otherwise `false`.
 *
...
impl MyHashSet {
    ...
    fn add(&mut self, key: i32) {
        ...
        if !self.buckets[bucket_idx].contains(&key) {
    ...
    fn remove(&mut self, key: i32) {
        let bucket_idx = self.get_bucket_index(key);
        self.buckets[bucket_idx].retain(|&x| x != key);
    }
