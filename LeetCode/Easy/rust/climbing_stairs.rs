...
fn climb_stairs(n: i32) -> i32 {
    fn number_of_ways_to_top(height: i32, max_steps: i32, memo: &mut HashMap<i32, i32>) -> i32 {
        if let Some(ways) = memo.get(&height) {
            return *ways;
        }
        ...
        let min_step = std::cmp::min(max_steps, height) + 1;

        for step in 1..min_step {
            number_of_ways += number_of_ways_to_top(height - step, max_steps, memo);
        }

        memo.insert(height, number_of_ways);
        number_of_ways
    }

    number_of_ways_to_top(n, 2, &mut HashMap::from([(0, 1), (1, 1)]))
}
 ...
 *
 * 1. We create an array `waysToTop` of size `height + 1` (n + 1) and fills it with `zeros`. This array will store the number of ways to reach each height from the bottom (0th index) up to the top stair (height-th index).
 *
...
pub fn climb_stairs(n: i32) -> i32 {
    let mut ways_to_top: HashMap<i32, i32> = HashMap::new();
    ways_to_top.insert(0, 1);
    ways_to_top.insert(1, 1);

    for current_height in 2..=n {
        let mut step = 1;
        while step <= 2 && step <= current_height {
            *ways_to_top.entry(current_height).or_insert(0) +=
                *ways_to_top.get(&(current_height - step)).unwrap_or(&0);
            step += 1;
        }
    }

    *ways_to_top.get(&n).unwrap()
}
 ...
 */
pub fn climb_stairs(n: i32) -> i32 {
    let mut max_steps: i32 = 2;
    let mut current_number_of_ways: i32 = 0;
    let mut ways_to_top: Vec<i32> = vec![1];

    for current_height in 1..=n {
        let start_of_window: i32 = current_height - max_steps - 1;
        let end_of_window: i32 = current_height - 1;

        if start_of_window >= 0 {
            current_number_of_ways -= ways_to_top[start_of_window as usize];
        }

        current_number_of_ways += ways_to_top[end_of_window as usize];
        ways_to_top.push(current_number_of_ways);
    }
