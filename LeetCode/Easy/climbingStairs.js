 ...
 */
const climbStairs = (n) => {
  return numberOfWaysToTop(n, 2, { 0: 1, 1: 1 });
};
 ...
 */
const climbStairs = (n) => {
  let waysToTop = new Array(n + 1).fill(0);
  waysToTop[0] = 1;
  waysToTop[1] = 1;

  for (let currentHeight = 2; currentHeight < n + 1; currentHeight++) {
    let step = 1;

    while (step <= 2 && step <= currentHeight) {
...
const climbStairs = (n) => {
  ...
  for (let currentHeight = 1; currentHeight < n + 1; currentHeight++) {
