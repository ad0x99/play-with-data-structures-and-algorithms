 ...
 *
 * Brute Force Approach: We will use 2 nested-loop to iterate over the elements in both arrays. If we found the first array's element in the second array, we will iterate over the remaining second array's elements to find the greater element of the current first array's element.
 *
 ...
 *
 *
 * 1. We create an empty hash map (`map`) to store the next greater element for each element encountered in `nums2`.
 * 2. We create an empty stack (`stack`) to keep track of elements seen in `nums2` during processing.
 ...
 * - 7.1: If the key (`num`) exists in the map, its corresponding value (the next greater element) is retrieved and added to the ans array.
 * - 7.2: If the key (`num`) doesn't exist in the map (meaning num wasn't present in nums2), `-1` is added to `ans` to indicate no greater element exists.
 *
 * 8. We return the `ans` array which contains the next greater element for each element in nums1 (or -1 if none exists).
 *
 * Time complexity: O(n + m), where n is the length of nums1 and m is the length of nums2. This is because we iterate through both arrays once to build the map of next greater elements.
 *
 * Space complexity: O(m) because we use a map to store the next greater elements for elements in nums2, and a stack to efficiently find the next greater element for each element in nums2. The space used by the map and stack is proportional to the length of nums2.
 */
const nextGreaterElement = (nums1, nums2) => {
  const map = {};
  const stack = [];
