 ...
 * 2. We iterate through the string s and updates the count for each character in the charCount Map.
...
const frequencySort = (s) => {
  ...
  s = s.split('');
  s.sort((a, b) => {
    const countA = charCount.get(a);
    const countB = charCount.get(b);
 ...
 *
 * 1. We create a map (`charCount`) to store the character frequencies and iterate through each character to count the frequency.
 * 2. We create a bucket array with size equal to the string `length + 1` to hold characters based on their frequencies.
 * 3. We iterate through `charCount` entries (key-value pairs) using `entries()`. For each character (`char`) and its frequency (`freq`), the character is pushed into the corresponding bucket at index freq in the bucket array.
 * 4. We create a empty string ans to store the sorted characters.
 * 5. We iterate through frequencies in descending order (`freq = s.length; freq >= 0; freq--`).
 * - 5.1: Inside the inner loop, we iterate through each character (char) in the current bucket (`bucket[freq]`). Instead of appending the character multiple times, char.repeat(`freq`) is used to create a string with the character repeated freq times (more efficient)The repeated character string is appended into the ans string.
 *
 * 6. We return the sorted string which is stored in the ans variable.
 *
...
const frequencySortBucketSort = (s) => {
  ...
  for (const [char, freq] of charCount.entries()) {
