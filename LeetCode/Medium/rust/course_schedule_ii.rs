...
impl TopologicalSort {
    ...
    fn sort_bfs(&mut self) -> Vec<i32> {
        ...
        while let Some(current_course) = queue.pop_front() {
            ...
            result.push(current_course as i32);
        ...
        if result.len() == self.num_courses {
            result
        } else {
            Vec::new()
        }
    ...
    fn sort_dfs(&mut self) -> Vec<i32> {
        ...
        fn dfs(
            ...
            if let Some(dependent_courses) = edge_map.get(&course) {
                for &dependent_course in dependent_courses {
                    ...
                    if !dfs(dependent_course, state, edge_map, result) {
                        return false;
                    }
                }
            }
        ...
        for course in 0..self.num_courses {
            ...
            if state[course] == 0 {
                if !dfs(course, &mut state, &self.edge_map, &mut result) {
                    return Vec::new(); // Cycle detected, return empty array
                }
            }
        }
...
impl Solution {
    pub fn find_order(num_courses: i32, prerequisites: Vec<Vec<i32>>) -> Vec<i32> {
        ...
        topo.sort_bfs() // or topo.sort_dfs()
    }
}
