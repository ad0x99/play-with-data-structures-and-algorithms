...
const countHousePlacements = (n) => {
  ...
  let empty = 1n; // Initialize the number of ways to place houses such that the last plot is empty
  let house = 1n; // Initialize the number of ways to place houses such that the last plot has a house

  // Iterate over the number of plots from 2 to n
  for (let i = 2; i <= n; i++) {
    // Swap the house and empty plots positions
    [empty, house] = [(house + empty) % mod, empty];
  }

  // The total number of ways for one side of the street
  let oneSideCombination = (empty + house) % mod;
  return (oneSideCombination * oneSideCombination) % mod;
};
