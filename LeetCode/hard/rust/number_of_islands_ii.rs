...
impl UnionFindGraph {
    ...
    fn find(&mut self, vertex: usize) -> usize {
        ...
        while vertex != root {
            ...
            self.parent[vertex] = root as i32;
            vertex = parent;
        }
...
impl Solution {
    pub fn num_islands2(m: i32, n: i32, positions: Vec<Vec<i32>>) -> Vec<i32> {
        let mut graph = UnionFindGraph::new(m as usize, n as usize);
        ...
        for pos in positions {
            ...
            // If the current cell is already part of an island,
            // we update result as the island doesn't change, and skip the current cell
            if grid[row][col] == 1 {
                result.push(count);
                continue;
            }
            ...
            let current_index = row * n as usize + col;

            // Traverse through 4 directions to connect the current cell to its neighbors
            for (row_offset, col_offset) in &directions {
                let new_row = (row as i32 + row_offset) as usize;
                let new_col = (col as i32 + col_offset) as usize;

                if new_row < m as usize && new_col < n as usize && grid[new_row][new_col] == 1 {
                    let neighbor_index = new_row * n as usize + new_col;
                    let u = graph.find(neighbor_index);
                    let v = graph.find(current_index);
